    ...
    if (format == "text") {
        ...
        for (const auto& e : history) {
            rows.push_back({fmt::to_string(e.id), fmt::to_string(e.shard_id), e.keyspace, e.table, e.compaction_type, format_compacted_at(e.compacted_at),
                    fmt::to_string(e.bytes_in), fmt::to_string(e.bytes_out), "", fmt::to_string(e.sstables_in), fmt::to_string(e.sstables_out),
                    fmt::to_string(e.total_tombstone_purge_attempt), fmt::to_string(e.total_tombstone_purge_failure_due_to_overlapping_with_memtable),
        ...
        static auto unpack_caller = [] <typename Function, typename T, size_t N> (Function&& f, const std::array<T, N> &args) {
            return [] <size_t... I> (Function&& f, const std::array<T, N> &args, std::index_sequence<I...>) {
                return f(args[I]...);
            }(std::forward<Function>(f), args, std::make_index_sequence<N>{});
        };
        ...
        const auto regular_row_format = unpack_caller(regular_row_formatter, max_column_length);
